...
// Importação da imagem da logo do aplicativo
...
export default function LoginScreen({ navigation }: any) {
  ...
  // Valida as credenciais digitadas e realiza o login se as informações coincidirem com as salvas
  ...
  // Registra um novo usuário no armazenamento local, impedindo a criação caso já exista uma conta ativa
  ...
  // Exibe uma caixa de confirmação para substituir as credenciais antigas pelas novas que foram digitadas
        ...
        { 
          ...
          onPress: async () => {
            ...
            if (loginLimpo !== '' && senhaLimpa !== '') {
              const userData = { login: loginLimpo, password: senhaLimpa };
              await AsyncStorage.setItem('@user_credentials', JSON.stringify(userData));
              Alert.alert("Sucesso", "Seus dados de acesso foram updated!");
            } else {
  ...
  // Interface gráfica estrutural da tela de autenticação
  return (
    <KeyboardAvoidingView 
    ...
    >
      <View style={styles.loginBox}>
        ...
        {/* Renderização da Logo adicionada */}
        <Image 
          source={LogoImg} 
          style={styles.logo} 
          resizeMode="contain" 
        ...
        {/* Campo para inserção do login do usuário */}
        <TextInput 
          style={styles.input}
          placeholder="Login" 
        ...
        {/* Campo mascarado para inserção da senha do usuário */}
        <TextInput 
          style={styles.input}
          placeholder="Senha"
          placeholderTextColor="#999"
        ...
        {/* Botão de envio para disparar o login */}
        <TouchableOpacity style={styles.button} onPress={handleLogin}>
          <Text style={styles.buttonText}>Entrar</Text>
        </TouchableOpacity>

        {/* Seção inferior contendo links alternativos de navegação e cadastro */}
